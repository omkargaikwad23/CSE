...
void A_StarAlgorithm::aStarSearch(){
    ...
    vector<pii>vectDir = {{0, 1}, {0, -1}, {1, 0}, {-1, 0}};
    ...
    while(!openList.empty()){
        ...
        for(auto dir : vectDir){
            int new_i = dir.first+i;
            int new_j = dir.second+j;
            double gNew, hNew, fNew;

            if(isDestination(new_i, new_j)){
                    cellInfo[new_i][new_j].parent_i = i;
                    cellInfo[new_i][new_j].parent_j = j;
                    printf("The milk is found\n");
                    tracePath(cellInfo);
                    foundDest = true;
                    return;
            }else if(closedList[new_i][new_j]==false && isOpen(new_i, new_j)){
                    gNew = cellInfo[i][j].g + 1.0;
                    hNew = calculateHval(new_i, new_j);
                    fNew = gNew + hNew;

                    // If it isn’t on the open list, add it to
                    // the open list or if it is better then update it
                    if(cellInfo[new_i][new_j].f==FLT_MAX || cellInfo[new_i][new_j].f > fNew){
                        openList.insert(mp(fNew, mp(new_i, new_j)));
                        cellInfo[new_i][new_j].g = gNew;
                        cellInfo[new_i][new_j].h = hNew;
                        cellInfo[new_i][new_j].f = fNew;
                        cellInfo[new_i][new_j].parent_i = i;
                        cellInfo[new_i][new_j].parent_j = j;
                    }
                }
